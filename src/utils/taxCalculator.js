 ...
 * @param {number} inputs.sideIncome - 副業月収（万円）
 * @param {number} inputs.sideExpense - 副業月間経費（万円）
...
export function calculate(inputs) {
  ...
  const sideIncomeMonthYen = sideIncome * 10000
  const sideExpenseMonthYen = sideExpense * 10000

  // 副業の稼働月数（開始月〜12月）
  const activeMonths = Math.max(0, 13 - startMonth)

  // 年間副業収入・経費・所得
  const annualSideIncome = sideIncomeMonthYen * activeMonths
  const annualSideExpense = sideExpenseMonthYen * activeMonths
  ...
  for (let month = 1; month <= 12; month++) {
    ...
    const monthlyGross = isActive ? sideIncomeMonthYen : 0
    const monthlyExpense = isActive ? sideExpenseMonthYen : 0
